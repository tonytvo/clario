...
The referenced `README.md` (preserved verbatim as the surrounding business context) describes Clario as follows:
...
Supporting business intent from `roadmap.md`:
...
**Note on requirement completeness:** The stated requirement ("we're looking to record the expense") is high-level and carries **no explicit acceptance criteria**. This analysis derives implied ACs from the README + roadmap and flags every derived assumption in the Risk & Gap Analysis. Scope disambiguation is the single most important open question (see below).

### Amendment — tech-stack direction change (2026-09-05)

The stakeholder amended the technical direction after the initial analysis:

- **Drop Supabase.** The project moves to a **local-first** architecture: data lives on the user's device and works fully offline.
- **CRDT-based sync across devices/users**, using a library from the Automerge / Yjs family. The **exact CRDT engine and sync transport are explicitly deferred** — "we will decide how exactly we do that later."
- **Keep the persistence/sync decision flexible** — switching the underlying technology should be as cheap and as late as possible.

The README's tech-stack table (Supabase / Postgres / Google Drive) is therefore **superseded for the persistence and authorization layers** and should be treated as historical context, not the target. This amendment reframes the Strategic Approach and Risk sections below; the Domain Concepts remain valid because they are technology-neutral (only their *current implementation* references — Supabase tables/RLS — are historical).

---

## Domain Concept Identification

Concept-driven exploration confirmed that **"record an expense" is not greenfield** — a working `add_expense` capability already exists end-to-end (Feature 1, "Add Expense via Chat"). Its **current implementation is Supabase-Postgres with RLS**, which the amendment now slates for replacement. The concepts below are the durable, technology-neutral domain; the "currently implemented as …" notes describe the code as it stands today and mark what must be re-homed behind a persistence port.
...
- **Group**: The container an expense belongs to; owns a default currency and a member roster. An expense cannot exist without a group. Currently `groups`. **Under local-first sync, the group is the natural unit of replication/sharing** (see Strategic Approach).
...
- **Receipt (link)**: An optional attachment to an expense, stored as a URL to the user's own cloud per ADR-001 — never as bytes on a server. Currently `receipt_links` + `attach_receipt` / `upload_receipt_from_path`. Related to recording only as a follow-on step. *Its cloud-Drive storage model sits in slight tension with "local-first" and is flagged as an open question below.*
...
- **Member selection (participant picker)**: "Split with group members" implies choosing real participants by identity. Today the web flow only asks "split N ways" and substitutes placeholder UUIDs — there is no selection of actual members. Unchanged by the stack shift, still the principal functional gap.
...
- **Move authorization from RLS into the domain/service layer.** Membership and "can record into this group" checks that Postgres RLS used to enforce now live in the application, invoked before a write. Visibility becomes a function of which group documents a user replicates. (This was already a flagged integrity gap; the stack shift makes it mandatory.)
- **General data flow (revised):** *transcript or form → validated tool input → domain service (invariant + membership checks, split allocation) → persistence port `.recordExpense()` → local CRDT adapter appends one change → balances derived on read → structured split breakdown returned.* Sync/replication happens out-of-band behind the port.
- **Keep the engine decision deferred but not blocking.** Build against the port with a minimal local adapter so recording works today; design the port's method shapes to be satisfiable by Automerge, Yjs, or any successor without signature changes.
...
- **Split allocation & custom-amount validation** (unchanged by the shift): deterministic remainder allocation; validate custom amounts (keys ⊆ participants, sum = total, all positive). → **Recommendation:** implement server/domain-side.
- **Participant identity in the web flow** (unchanged): replace placeholder UUIDs with real member selection sourced from group membership. → **Recommendation:** required for a real split; may depend on group management existing.
...
- **Keep Supabase / build directly on a database client** — rejected: contradicts the stakeholder's local-first + swap-late directive and re-welds the domain to one vendor.
- **Swap Supabase for another cloud DB (e.g. Postgres elsewhere) without a port** — rejected: still cloud-dependent, not local-first, and leaves the same coupling that made this pivot expensive.
- **Pick the CRDT engine (Automerge or Yjs) now** — rejected per the explicit instruction to decide later; committing early would leak engine specifics into the domain before the port stabilizes.
...
- **Meaning of the pivot's timeline**: is the port + local-first re-homing part of *this* "record expense" work, or a preceding foundational task? Recording cannot be trustworthy on local-first until the port and domain-layer authz exist.
...
- **Receipt storage under local-first**: ADR-001 stores receipts in the user's Google Drive (a cloud dependency). Does local-first extend to receipts (local blob storage, synced), or do receipt links remain cloud-based? Unresolved.
...
- **Clock skew across devices** for the default date / event ordering (CRDTs use logical, not wall, clocks — but the human-facing `date` still needs sensible defaulting).
...
- **Loss of RLS as an authorization backstop (security)** — *Impact:* without Postgres RLS, any gap in app-layer checks exposes or corrupts data. *Mitigation:* thorough domain-layer membership/participation checks with dedicated tests; treat replication scope as the visibility boundary.
- **Port designed with SQL assumptions (architectural — defeats the goal)** — *Impact:* a leaky, query-builder-shaped port that a CRDT adapter can't satisfy, forcing a rewrite exactly when swapping. *Mitigation:* document/aggregate-oriented port validated against both a trivial local adapter and a sketch of a CRDT adapter before locking it.
...
- **Receipt cloud dependency vs. local-first (consistency of vision)** — *Impact:* a "local-first" app that still needs Google Drive for receipts. *Mitigation:* decide receipt strategy explicitly (out of scope for recording, but flag).
...
No explicit ACs were provided. The table assesses **derived** ACs (from README + roadmap), now re-scored against the local-first / CRDT direction. Each should be confirmed with the stakeholder.
...
| 8 | Concurrent/offline recordings merge without loss or invariant violation | Partial | New requirement from the pivot; addressed by append-only event model |
...
| 10 | A receipt can be associated with the recorded expense | Yes (separate step) | Existing `attach_receipt`; but local-first receipt storage is an open question |
